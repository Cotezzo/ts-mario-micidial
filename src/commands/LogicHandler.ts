...
const logicHandler: Commands<Command> = {
    ...
    "langs, languages": {
        ...
        fn: () => {
            ...
            array = Object.entries(Talker.supportedIbmVoices);

            // Codice rubato dal Doujinshi bot per creare tre colonne in cui elencare gli argomenti
            resto = array.length % 3;
            elemPerColon = (array.length - resto) / 3;
        
            lungs = [elemPerColon + (resto ? 1 : 0)];
            lungs.push(lungs[0] + elemPerColon + (resto == 2 ? 1 : 0));
            lungs.push(lungs[1] + elemPerColon);
        
            fields.push({ name: `Ibm Voices`, value: parseFieldsIbm(array.slice(0, lungs[0])), inline: true });
            if (elemPerColon) fields.push({ name: `­`, value: parseFieldsIbm(array.slice(lungs[0], lungs[1])), inline: true });
            if (elemPerColon) fields.push({ name: `­`, value: parseFieldsIbm(array.slice(lungs[1], lungs[2])), inline: true });
    ...
    */
    ...
    "say": {
        ...
        fn: (risp: Message | CommandInteraction, text) => {
            ...
            if (talker.deleteMessages && risp instanceof Message) risp.delete().catch(() => {});    //Se è impostato per farlo, elimina il messaggio
        }
    }
};
